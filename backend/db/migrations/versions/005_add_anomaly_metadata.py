...
def upgrade() -> None:
    ...
    op.add_column("anomalies", sa.Column("anomaly_metadata", postgresql.JSONB, nullable=True))
    ...
    op.create_check_constraint(
        "ck_anomaly_severity", "anomalies", "severity IN ('low', 'medium', 'high', 'critical', 'info', 'warning')"
    )
...
def downgrade() -> None:
    ...
    op.create_check_constraint("ck_anomaly_severity", "anomalies", "severity IN ('low', 'medium', 'high', 'critical')")

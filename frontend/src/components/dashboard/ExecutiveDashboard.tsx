...
import { useAlertSummary, useForecasts, useStores, useForecastAccuracy, useInventorySummary } from '@/hooks/useShelfOps'
...
export default function ExecutiveDashboard() {
    ...
    const kpis = useMemo(() => {
        ...
        const resolvedAlerts = alertSummary?.resolved ?? 0
        ...
        // Forecast accuracy: average (100 - MAPE) from the accuracy endpoint
        const avgAccuracy = accuracyData.length > 0
            ? Number(
                (accuracyData.reduce((sum, a) => sum + Math.max(0, 100 - a.avg_mape), 0) / accuracyData.length).toFixed(1)
            )
            : null
    ...
    const chartData = useMemo(() => {
        const byDate = new Map<string, { predicted: number; count: number }>()
        forecasts.forEach(f => {
            const dateKey = f.forecast_date
            const existing = byDate.get(dateKey) ?? { predicted: 0, count: 0 }
            existing.predicted += f.forecasted_demand
            existing.count += 1
